...
class Producto : AppCompatActivity() {
    private val arregloProductos = BaseDatosMemoria.arregloProductos
    ...
    private fun eliminarProductoSeleccionado() {
        BaseDatosMemoria.arregloProductos.removeAt(posicionItemSeleccionado)
        actualizarListaProductos()
    ...
    private fun obtenerProductosFiltrados(): List<EntrenadorProducto> {
        return arregloProductos.filter { it.categoriaID == categoriaActual.id }
    }
}
